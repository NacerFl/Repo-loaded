...
contract NODERewardManager is PaymentSplitter {
	...
	bool private openCreateFree = false;
	bool private openLevelUp = false;


	uint256 public rewardsTEST;
	...
	function createNodeFree(string memory nodeTypeName, uint256 count) public {
		require(openCreateFree, "Node creation Free not authorized yet");
		require(_doesNodeTypeExist(nodeTypeName), "nodeTypeName does not exist");
		address sender = msg.sender;
		require(
            sender != futurUsePool && sender != distributionPool,
	...
	function cashoutAll() public {
		...
		// //test
		// IERC20(_polarTokenAddress).approve(address(this), rewardAmount);
	...
	function updateOpenCreateFree(bool value) external onlySentry {
        openCreateFree = value;
    }
    ...
    function swapTokensForEth(uint256 tokenAmount) public pure {
